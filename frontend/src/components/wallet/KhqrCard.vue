...
// Mirrors `assets/KHQR_card.svg`'s artwork (header/dashed-divider/rounded card), redrawn inline
// so the amount, merchant name, and QR image can be layered into the same 442x622 coordinate
// space instead of guessing pixel offsets over a flat <img>.
defineProps<{
  amountUsd: number
...
<template>
  <svg viewBox="0 0 442 622" fill="none" xmlns="http://www.w3.org/2000/svg" class="mx-auto w-full max-w-85">
    <g filter="url(#khqr-card-shadow)">
      <g clip-path="url(#khqr-card-clip)">
        ...
        <text x="71" y="150" class="fill-gray-500 text-lg font-medium">SquadUp</text>
        <text x="71" y="195" class="fill-gray-900 text-[48px] font-semibold">
          ${{ amountUsd.toFixed(2) }}<tspan dx="8" class="fill-gray-500 text-[15px] font-medium">USD</tspan>
        </text>

        <image v-if="qrDataUrl" :href="qrDataUrl" x="71" y="258" width="300" height="300" />
      </g>
    </g>
    <defs>
      <filter id="khqr-card-shadow" x="0" y="0" width="442" height="622" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
        ...
        <feBlend mode="normal" in2="BackgroundImageFix" result="khqr-card-shadow-blend" />
        <feBlend mode="normal" in="SourceGraphic" in2="khqr-card-shadow-blend" result="shape" />
      </filter>
      <clipPath id="khqr-card-clip">
        <rect x="21" y="21" width="400" height="580" rx="24" fill="white" />
      </clipPath>
    </defs>
  </svg>
</template>
