...
const player = computed(() => mockPlayers.find((p) => p.id === route.params.id) ?? mockPlayers[0]!)
...
const detail = computed(
  () =>
    ...
    fallbackServiceDetail(service.value.name, service.value.priceCoins, service.value.priceUnit, service.value.promoBadge),
)
const reviews = computed(() => profile.value.reviews[serviceId.value] ?? [])

const selectedTypeIndex = ref(0)
watch(detail, () => (selectedTypeIndex.value = 0))
const selectedType = computed(() => detail.value.serviceTypes[selectedTypeIndex.value] ?? detail.value.serviceTypes[0]!)

const tags = computed(() => [...detail.value.platforms, ...detail.value.styles, ...player.value.languages])
    ...
    <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
      <div class="flex flex-col gap-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="inline-flex items-center gap-1 text-xs text-slate-400">
                  ...
                  {{ detail.rating ? detail.rating.toFixed(1) : '--' }} ({{ detail.servedCount.toLocaleString() }})
                </p>
              </div>
            </div>
            <UButton color="primary" variant="soft" size="sm" class="rounded-full" @click="router.push('/messages')">
              Message
            </UButton>
          </div>

          <div v-if="tags.length" class="mt-4 flex flex-wrap gap-2">
            <UBadge v-for="tag in tags" :key="tag" color="neutral" variant="soft" size="sm" class="rounded-full text-xs">
              {{ tag }}
            </UBadge>
          </div>
        </div>
          ...
          <div class="mt-3 flex flex-col gap-2">
            ...
            >
              <span
                ...
                :class="index === selectedTypeIndex ? 'bg-brand-500 ring-brand-500' : 'ring-slate-500'"
              >
        ...
        <div v-if="detail.whatsIncluded.length" class="rounded-xl bg-gray-800/70 p-5">
          ...
          <ul class="mt-3 flex flex-col gap-2.5">
            <li v-for="item in detail.whatsIncluded" :key="item" class="flex items-center gap-2 text-sm text-slate-300">
      ...
      <div class="h-fit rounded-xl bg-gray-800/70 p-5 lg:sticky lg:top-24">
        <div class="flex flex-wrap items-center gap-2">
          ...
          <UBadge v-if="selectedType.promoBadge" color="primary" variant="soft" size="sm" class="rounded-full text-xs text-brand-500">
        ...
        <UButton color="primary" block size="lg" class="mt-4 rounded-full" @click="bookingOpen = true">
          Book now
        </UButton>
        <UButton
          color="primary"
        ...
        <p class="mt-3 text-center text-xs text-slate-400">Avg response {{ detail.avgResponseTime }}</p>
      </div>
    </div>
